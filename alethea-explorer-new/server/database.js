...
export class BlockchainDatabase {
  ...
  getChainsCount() {
    const stmt = this.db.prepare('SELECT COUNT(DISTINCT chain_id) as count FROM blocks');
  ...
  getChainBlockCount(chainId) {
    const stmt = this.db.prepare('SELECT COUNT(*) as count FROM blocks WHERE chain_id = ?');
