...
pub struct Voter {
    ...
    /// Withdrawable balance - rewards and unstaked tokens ready to claim
...
pub struct Query {
    ...
    /// Selected voters for this query (by stake × reputation power)
    pub selected_voters: Vec<String>,
...
impl Voter {
    ...
    fn from_voter_info(
        ...
        Self {
            ...
            // Include withdrawable balance for claimed rewards and unstaked tokens
...
impl Query {
    ...
    fn from_state_query(
        ...
        // Convert selected voters to strings
        let selected_voters: Vec<String> = query.selected_voters.iter()
            .map(|v| format!("{}", v))
            .collect();
        ...
        Self {
            ...
            selected_voters,
            commit_count: query.commits.len() as u32,
        }
    }
...
impl Statistics {
    ...
    /// 
    /// FIX: Now properly calculates active_voters, average_reputation, and total_locked_stake
    async fn from_state(state: &OracleRegistryV2) -> Self {
        ...
        // FIX: Properly iterate through all voters to calculate accurate statistics
        let mut active_voters: u64 = 0;
        let mut total_reputation: u64 = 0;
        let mut total_locked_stake_value: u128 = 0;
        let mut voter_count_for_avg: u64 = 0;
        
        // Get all voter indices and iterate
        if let Ok(voter_indices) = state.voters.indices().await {
            for chain_id in voter_indices {
                if let Some(voter_info) = state.get_voter(&chain_id).await {
                    voter_count_for_avg += 1;
                    
                    // Count active voters
                    if voter_info.is_active {
                        active_voters += 1;
                    }
                    
                    // Sum reputation for average calculation
                    total_reputation += voter_info.reputation as u64;
                    
                    // Sum locked stakes
                    let locked_value: u128 = voter_info.locked_stake.into();
                    total_locked_stake_value += locked_value;
                }
            }
        }
        
        // Calculate average reputation
        let average_reputation = if voter_count_for_avg > 0 {
            total_reputation as f64 / voter_count_for_avg as f64
        } else {
            50.0 // Default reputation
        };
        
        // FIX: Use from_attos since locked_stake values are in attos
        let total_locked_stake = linera_sdk::linera_base_types::Amount::from_attos(total_locked_stake_value);
        
        // Calculate average stake per voter
        // FIX: Use from_attos since total_stake is in attos
        let average_stake = if total_voters > 0 {
            let stake_value: u128 = total_stake.into();
            linera_sdk::linera_base_types::Amount::from_attos(stake_value / total_voters as u128)
        } else {
...
impl MutationRoot {
    ...
    /// 
    /// This mutation sends a cross-chain message to withdraw stake on the target chain.
    /// The voter's chain ID will be the sender's chain ID (automatically detected).
    /// 
    /// # Arguments
    /// * `target_chain` - The chain ID where the registry is deployed (app chain)
    ...
    async fn send_withdraw_stake_message(
        ...
        // Create SendWithdrawStakeMessage operation
        let operation = Operation::SendWithdrawStakeMessage {
            target_chain: target_chain_id,
            amount: withdraw_amount,
        };
