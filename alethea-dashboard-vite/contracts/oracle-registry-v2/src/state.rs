...
impl OracleRegistryV2 {
    ...
    /// 
    /// Returns all votes stored in the query's votes map.
    /// This properly fetches the query and extracts the votes from it.
    pub async fn get_query_votes(&self, query_id: u64) -> Vec<Vote> {
        // Fetch the query by ID
        match self.get_query(query_id).await {
            Some(query) => {
                // Extract votes from the query's votes map
                query.votes.values().cloned().collect()
            },
            None => {
                // Query not found, return empty vector
                Vec::new()
            }
        }
    }
    ...
    pub async fn get_available_stake(&self, voter_chain: &ChainId) -> Amount {
        match self.get_voter(voter_chain).await {
            Some(info) => {
                ...
                Amount::from_attos(stake_value.saturating_sub(locked_value))
            },
            None => Amount::ZERO,
        }
    }
