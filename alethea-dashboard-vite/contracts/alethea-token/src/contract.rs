    ...
    async fn execute_message(&mut self, message: Self::Message) {
        match message {
            ...
            Message::MintReward {
                ...
                // SECURITY: Verify sender is authorized registry
                // Only the configured registry application can mint rewards
                let registry_app_id = self.state.registry_app_id.get().clone();
                
                if registry_app_id.is_none() {
                    eprintln!(
                        "SECURITY: MintReward rejected - no registry configured. Attempted to mint {} to {:?}",
                        amount, to
                    );
                    return;
                }
                
                // Verify the message comes from the authorized registry
                // In Linera, cross-chain messages are authenticated by the runtime
                // SECURITY: Fail closed - reject if we can't verify the sender
                if self.runtime.message_is_bouncing().unwrap_or(false) {
                    eprintln!("SECURITY: MintReward rejected - message is bouncing");
                    return;
                }
                
                // The authenticated_caller_id() returns the application that sent the message
                let caller_id = match self.runtime.authenticated_caller_id() {
                    Some(id) => id,
                    None => {
                        // SECURITY: No authenticated caller - reject the operation
                        eprintln!(
                            "SECURITY: MintReward rejected - no authenticated caller. Amount: {}, To: {:?}",
                            amount, to
                        );
                        return;
                    }
                ...
                let expected_registry = registry_app_id.unwrap();
                if caller_id != expected_registry {
                    eprintln!(
                        "SECURITY: MintReward rejected - unauthorized sender. Expected {:?}, got {:?}",
                        expected_registry, caller_id
                    );
                    return;
                }
                
                eprintln!(
                    "MintReward authorized: {} tokens to {:?} on chain {}",
                    amount, to, to_chain
                );
            ...
            Message::BurnSlash {
                ...
                // SECURITY: Verify sender is authorized registry
                // Only the configured registry application can burn (slash) tokens
                let registry_app_id = self.state.registry_app_id.get().clone();
                
                if registry_app_id.is_none() {
                    eprintln!(
                        "SECURITY: BurnSlash rejected - no registry configured. Attempted to burn {} from {:?}",
                        amount, from
                    );
                    return;
                }
                ...
                // SECURITY: Fail closed - reject if we can't verify the sender
                if self.runtime.message_is_bouncing().unwrap_or(false) {
                    eprintln!("SECURITY: BurnSlash rejected - message is bouncing");
                    return;
                }
                ...
                let caller_id = match self.runtime.authenticated_caller_id() {
                    ...
                    None => {
                        // SECURITY: No authenticated caller - reject the operation
                        eprintln!(
                            "SECURITY: BurnSlash rejected - no authenticated caller. Amount: {}, From: {:?}",
                            amount, from
                        );
                        return;
                    }
                ...
                if caller_id != expected_registry {
                    eprintln!(
                        "SECURITY: BurnSlash rejected - unauthorized sender. Expected {:?}, got {:?}",
                        expected_registry, caller_id
                    );
                    return;
                }
                ...
                eprintln!(
                    "BurnSlash authorized: {} tokens from {:?} on chain {}",
                    amount, from, from_chain
                );
                ...
                if burn_amount > Amount::ZERO {
                    ...
                    eprintln!(
                        "BurnSlash completed: {} tokens burned from {:?}. New balance: {}",
                        burn_amount, from, balance.saturating_sub(burn_amount)
                    );
                } else {
                    eprintln!(
                        "BurnSlash: No tokens to burn from {:?}. Balance: {}",
                        from, balance
                    );
                }
            }
