...
export const TokenProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const loadTokenInfo = useCallback(async () => {
        ...
        try {
            // Alethea token contract provides tokenInfo query
            ...
            if (data?.tokenInfo) {
                setTokenInfo({
                    name: data.tokenInfo.name || 'Alethea Token',
                    symbol: data.tokenInfo.symbol || 'ALTH',
                    decimals: data.tokenInfo.decimals || 18,
                    totalSupply: data.tokenInfo.totalSupply || '1000000000',
                    totalMinted: data.tokenInfo.totalMinted || '1000000000',
                    totalBurned: data.tokenInfo.totalBurned || '0',
                });
            } else {
                // Fallback if tokenInfo not available
                setTokenInfo({
                    name: 'Alethea Token',
                    symbol: 'ALTH',
                    decimals: 18,
