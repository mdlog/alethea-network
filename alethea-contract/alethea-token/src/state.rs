...
pub struct AletheaTokenState {
    ...
    // ===== SECURITY FEATURES =====
    /// Authorized registries that can receive stakes
    pub authorized_registries: SetView<ApplicationId>,
    /// Per-registry stake tracking
    pub registry_stakes: MapView<ApplicationId, Amount>,
    /// Per-user total stake tracking (across all registries)
    pub user_total_stakes: MapView<AccountOwner, Amount>,
}
