...
# Register Voter 1 (call registerVoter from Voter 1's endpoint)
echo "Registering Voter 1: $VOTER_1_ID"
curl -X POST "http://localhost:8080/chains/$CHAIN_ID/applications/$VOTER_1_ID" \
  -H "Content-Type: application/json" \
  -d '{"query":"mutation { registerVoter(stake: \"1000\") }"}'

echo ""

# Register Voter 2 (call registerVoter from Voter 2's endpoint)
...
# Register Voter 3 (call registerVoter from Voter 3's endpoint)
