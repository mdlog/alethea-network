...
echo "=== Voting Results Check ==="

echo "Voter 1 Status (after voting):"
curl -s "http://localhost:8080/chains/$CHAIN_ID/applications/$VOTER_1_ID" \
  -H "Content-Type: application/json" \
  -d '{"query":"{ status { stake reputation totalVotes correctVotes accuracyRate } activeVotesCount }"}' | jq .

echo ""
echo "Voter 2 Status (after voting):"
curl -s "http://localhost:8080/chains/$CHAIN_ID/applications/$VOTER_2_ID" \
  -H "Content-Type: application/json" \
  -d '{"query":"{ status { stake reputation totalVotes correctVotes accuracyRate } activeVotesCount }"}' | jq .

echo ""
echo "Voter 3 Status (after voting):"
curl -s "http://localhost:8080/chains/$CHAIN_ID/applications/$VOTER_3_ID" \
  -H "Content-Type: application/json" \
  -d '{"query":"{ status { stake reputation totalVotes correctVotes accuracyRate } activeVotesCount }"}' | jq .

echo ""
echo "Registry Protocol Stats:"
