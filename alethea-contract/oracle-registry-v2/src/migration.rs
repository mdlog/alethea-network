...
use crate::state::{Query, QueryStatus, QuerySource, DecisionStrategy, Vote, VotingPhase};
...
impl OldMarketData {
    ...
    pub fn to_query(&self, new_id: u64, creator: AccountOwner) -> Result<Query, String> {
        ...
        Ok(Query {
            ...
            // Metadata fields (none for migrated queries)
            title: None,
            category: None,
            context: None,
            ...
            metadata_url: None,
            external_id: None,
            // Source DApp tracking (migrated queries - internal)
            source_app_id: None,
            ...
            // Hybrid model fields (migrated queries - no bond)
    ...
    pub fn convert_votes(&self, voter_mapping: &VoterMapping) -> Result<Vec<(ChainId, Vote)>, String> {
        ...
        for old_vote in votes {
            ...
            let vote = Vote {
                ...
                voting_power: Amount::ZERO, // Migration: old votes don't have voting power tracked
            };
